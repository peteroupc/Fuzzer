...
namespace Fuzzer
{
	class MainClass
	{
		...
		public static void WriteFuzzedFileWithHeader(byte[] startbytes, Random rnd, string outputFile){
		...
		byte[] bytes, Random rnd, string outputFile, int fuzzStart, int frequency){
			int mode=rnd.Next(3);
			...
			using(FileStream fs=new FileStream(outputFile,FileMode.Create)){
				...
				if(realSize>bytes.Length){
					byte[] randomBytes=new byte[realSize-bytes.Length];
		...
		public static int Main(string[] args)
		{
			try {
				...
				XmlConfigFile config=XmlConfigFile.Create(
				   (args.Length>0) ? args[0] : "fuzzer.xml","fuzzer");
				...
				config.GetValue("validFilesPath"),config.GetValue("validFilesPattern","*.*"));
			...
			int fuzzOffset=config.GetValueAsInt32("fuzzOffset",0);
				...
				for(int i=0;i<Math.Max(0,iterations);i++){
					...
					path,i.ToString("d5",System.Globalization.CultureInfo.InvariantCulture)+"."+extension);
